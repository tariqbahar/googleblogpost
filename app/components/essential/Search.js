...
const Search = ({ searchOpen, toggleSearch }) => {
  ...
  // Fetch posts when search opens (initial load with empty query)
  useEffect(() => {
    if (searchOpen) {
      fetchPosts("");
    }
  ...
  // Fetch posts dynamically when searchTerm changes (with debounce)
  useEffect(() => {
  ...
  // Categories based on all posts
  const categories = popularCategories(posts);

  // Displayed posts: show recent (first 3) if no search term, otherwise show all fetched posts
  const displayedPosts = searchTerm ? posts : posts.slice(0, 3);
      ...
      <div className="[&>div]:px-3 sm:[&>div]:px-8 [&>div]:py-5 sm:[&>div]:py-12 [&>div]:rounded-lg [&>div]:md:rounded-xl w-full max-w-7xl mx-auto has-transition pointer-events-none [&>div]:pointer-events-auto z-50">
        <div>
          <label htmlFor="search" className="relative">
            <svg
              className="absolute -top-[2px] sm:-top-2 left-0 h-5 sm:h-7 w-5 sm:w-7"
              viewBox="0 0 21 21"
              fill="none"
                  ...
                  {searchTerm ? (
                    displayedPosts.length > 0 ? (
                      <>
                        Found{" "}
                        <span>
                          {displayedPosts.length.toString().padStart(2, "0")}
                        </span>{" "}
